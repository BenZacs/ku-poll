...
class QuestionModelTests(TestCase):
    """Test condition in th question model."""
...
class QuestionIndexViewTests(TestCase):
    ...
    def test_future_question(self):
        ...
        create_question(question_text="Future question.", pub_date=30, end_date=32)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(response.context['latest_question_list'], ['<Question: Future question.>'])

    def test_future_question_and_past_question(self):
        """Even if both past and future questions exist, only past questions are displayed."""
        create_question(question_text="Past question.", pub_date=-30, end_date=-15)
        create_question(question_text="Future question.", pub_date=30, end_date=40)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
    ...
    def test_two_past_questions(self):
        ...
        create_question(question_text="Past question 1.", pub_date=-25, end_date=-15)
        create_question(question_text="Past question 2.", pub_date=-30, end_date=-1)
        response = self.client.get(reverse('polls:index'))
        self.assertQuerysetEqual(
            response.context['latest_question_list'],
...
class QuestionDetailViewTests(TestCase):
    """Test quesion detail in the detail page."""

    def test_future_question(self):
        """The detail view of a question with a pub_date in the future returns a 404 not found."""
        future_question = create_question(
            question_text='Future question.', pub_date=5, end_date=15)
    ...
    def test_past_question(self):
        ...
        past_question = create_question(
            question_text='Past Question.', pub_date=-15, end_date=-5)
